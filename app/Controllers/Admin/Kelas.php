...
class Kelas extends BaseController
{
    ...
    public function store()
    {
        ...
        try {
            // Model PracticumClassModel punya $useTimestamps = true,
            // jadi created_at & updated_at diisi otomatis
            $classData = [
            ...
            // Assign lecturer (bisa null/kosong)
            $lecturerId = $this->request->getPost('lecturer_id');
            $this->classLecturerModel->assignLecturer($classId, $lecturerId ?: null, 'pengampu');

            // Assign assistant (bisa null/kosong)
            $assistantId = $this->request->getPost('assistant_id');
            $this->classAssistantModel->assignAssistant($classId, $assistantId ?: null, true);
    ...
    public function update()
    {
        ...
        try {
            $classData = [
                ...
                'status'           => $this->request->getPost('status'),
                'description'      => $this->request->getPost('description'),
                // updated_at dihandle otomatis oleh model ($useTimestamps = true)
            ];

            $this->practicumClassModel->update($classId, $classData);

            // Update lecturer (bisa null/kosong untuk menghapus)
            $lecturerId = $this->request->getPost('lecturer_id');
            $this->classLecturerModel->assignLecturer($classId, $lecturerId ?: null, 'pengampu');

            // Update assistant (bisa null/kosong untuk menghapus)
            $assistantId = $this->request->getPost('assistant_id');
            $this->classAssistantModel->assignAssistant($classId, $assistantId ?: null, true);
